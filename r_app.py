...
st.markdown(
    "Upload a **document image or PDF**, I'll extract the text and **read it aloud**. "
...
ocr_lang = st.sidebar.selectbox(
    "OCR Language (for images, Tesseract code):",
...
st.sidebar.info(
    "💡 Tip: For images, use clear, well-lit photos. For PDFs, text is extracted directly."
)

# ====== FILE UPLOAD (IMAGES + PDF) ======
st.subheader("1️⃣ Upload Image or PDF for Reading")
...
if uploaded_file is not None:
    ...
    if is_image:
        ...
        # --- Handle PDF: Extract text directly (no OCR) ---
        ...
        if ocr_text.strip():
            ...
            if autorun_ocr_speech:
                speak_in_browser("Reading document from uploaded PDF. " + ocr_text)
        else:
            st.warning("No readable text detected in the PDF.")
            if autorun_ocr_speech:
                speak_in_browser("No readable text detected in the PDF.")
    else:
...
# Note: Streamlit doesn't support direct CSS style param in text_area, but we can still
# control readability via font size in general settings if you want more custom styling.
ocr_text = st.text_area(
    "Output Text (you can edit this before reading):",
    value=ocr_text,
    height=220,
    key="ocr_output",
)
...
if st.button("🔊 Read Typed Text Aloud"):
    speak_in_browser("Reading typed text. " + (typed_text or ""))

st.markdown("---")
st.markdown("✅ Built with **Streamlit + Tesseract + PyPDF2 + Browser TTS**")
